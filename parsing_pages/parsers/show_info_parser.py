...
class ShowInfoParser:
    ...
    def get_id(self) -> ShowId:
        id = self.show_soup.find_all("link", attrs={"href": re.compile(self.SHOW_URL_TEMP)})[0]
        id = int(id["href"].split("/")[-2])
        return ShowId(id=id)

    def get_titles(self) -> Titles:
        # TODO preprocess values
        russian_title: str = self.show_soup.find_all("h1", attrs={"class": re.compile("styles_title")})[0].get_text()
        original_title: str = \
            self.show_soup.find_all("span", attrs={"class": re.compile("styles_originalTitle")})[
                0].get_text() if self.show_soup.find_all("span",
                                                         attrs={"class": re.compile("styles_originalTitle")}) else ""

        return Titles(russian_title, original_title)

    def get_actors(self) -> Cast:
        actor_soups = [tag_ for tag_ in self.show_soup.find_all("ul", attrs={"class": re.compile("styles_list")})]
    ...
    def get_rating(self) -> UserRating:
        ...
        rating_kinopoisk = self.show_soup.find_all("a", attrs={"class": re.compile("film-rating-value")})
        rating_imdb = self.show_soup.find_all("span", attrs={"class": re.compile("styles_valueSection")})

        if rating_kinopoisk:
            rating_kinopoisk = rating_kinopoisk[0].get_text()

            rating_count_kinopoisk = self.show_soup.find_all("span", attrs={"class": re.compile("styles_count")}
                                                             )[0].get_text()
        else:
        ...
        if rating_imdb:
            ...
            rating_count_imdb = self.show_soup.find_all("div", attrs={"class": re.compile("film-sub-rating")}
                                                        )[0].find("span",
                                                                  attrs={
    ...
    def get_synopsis(self) -> Synopsis:
        ...
        synopsis_list = self.show_soup.find_all("div", attrs={"class": re.compile("styles_filmSynopsis")})
        if synopsis_list:
    ...
    def get_critics_rating(self) -> CriticsRating:
        ...
        critics = self.show_soup.find_all("div", attrs={"class": re.compile("styles_ratingBar")})
